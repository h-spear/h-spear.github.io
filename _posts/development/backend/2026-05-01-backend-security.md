...
하지만 이처럼 단순한 순차적 숫자를 PK로 외부에 노출할 경우, **객체에 대한 접근 권한 검증이 제대로 이루어지지 않았을 때 다른 사용자의 데이터를 추측하여 접근하는 공격으로 이어질 수 있습니다.**    
...
이처럼 사용자가 전달한 식별자를 통해 객체에 직접 접근하면서, 해당 객체에 대한 접근 권한을 제대로 검증하지 않는 경우 **IDOR(Insecure Direct Object Reference)** 취약점이 발생할 수 있습니다.   
...
예를 들어 주문 ID가 `1000`번이면 총 주문량이 1000건이라는 사실이 외부에 그대로 드러나게 됩니다.


### **프로젝트에서의 판단**

IDOR의 근본적인 해결책은 ID를 예측 불가능하게 만드는 것이 아니라, **서버에서 접근 권한을 검증하는 것**입니다.   
다만 개발자가 접근 제어 로직을 누락할 수 있기 때문에, 예측하기 어려운 식별자를 안전장치로 두기로 했습니다.   

그래서 **엔티티의 성격**과 **"해당 ID가 클라이언트(외부)에 노출되는가?"**를 기준으로 ID 방식을 고민하게 되었습니다.  
크게 **주문, 상품, 유저** 엔티티로 나누어 제가 판단한 기준과 적용 과정을 설명하겠습니다.  

#### **1) 주문(Order): <span style="background-color: yellow;">외부 노출용 식별자 도입</span>**

주문 정보는 주문 내역 조회, 배송 조회 등 자주 노출되기 때문에, 외부에서 예측하기 어려운 식별자가 필요하다고 판단했습니다.  
...
상품 정보는 누구나 볼 수 있는 **공개 데이터**이고, 현재 서비스에서는 상품 ID를 통해 접근할 수 있는 데이터 역시 공개 범위로 제한되어 있기 때문에, 외부 식별자를 별도로 도입하지 않고 기존 `AUTO_INCREMENT` 방식을 유지했습니다.  
...
해당 프로젝트에서는 JWT 방식으로 인증을 처리하고 있습니다. JWT의 payload에는 사용자를 식별하기 위한 `userId`가 포함되어 있습니다.   

**서버는 요청 파라미터나 URL을 통해 전달받은 `userId`를 신뢰하지 않습니다.**   
인증이 필요한 API에서는 먼저 전달받은 JWT의 서명을 검증하고, **검증된 토큰의 payload에서 `userId`를 추출해 현재 요청을 보낸 사용자를 식별합니다.**  

따라서 `내 주문 조회`와 같은 API에서 클라이언트가 요청 파라미터에 다른 사용자의 `userId`를 전달하더라도, 서버는 해당 값을 사용하지 않습니다.   
**JWT 검증을 통해 확인된 사용자의 `userId`만을 기준으로 주문을 조회하기 때문에, 클라이언트가 요청 파라미터의 `userId`를 변경하는 방식으로 다른 사용자의 주문을 조회할 수 없도록 설계했습니다.**

```java
// @LoginUser: 검증된 JWT 토큰에서 서버가 직접 userId를 추출
// 클라이언트가 요청 파라미터로 전달한 userId는 사용하지 않음
...
즉, 이 경우에는 유저 ID가 1, 2, 3처럼 순차적으로 증가하더라도 **클라이언트가 요청에 포함된 `userId`를 변경해 다른 사용자의 데이터를 조회하는 구조가 아니기 때문에,** 유저 식별자를 별도의 외부 ID로 변경하지 않았습니다.

다만, JWT를 사용한다고 해서 IDOR 취약점이 자동으로 방지되는 것은 아닙니다. 특정 주문과 같이 리소스를 직접 식별하는 API에서는 인증된 사용자가 해당 리소스에 접근할 권한이 있는지 서버에서 별도로 검증해야 합니다.

> 물론 JWT Secret이 탈취될 경우, 공격자가 임의의 `userId`를 포함한 정상적인 서명의 토큰을 위조할 수 있습니다.   
...
|컬럼|암호화 방식|이유|
|---|---|---|
|이름, 전화번호, 이메일, 주소(우편번호·상세주소)|양방향 암호화|화면에 복호화해서 표시해야 함|
...
#### **검색 문제와 해결책 (검색용 해시 컬럼 도입)**
단, 양방향 암호화를 적용하면 암호화된 값은 `WHERE email = ?` 같은 일반적인 쿼리로 검색할 수 없게 됩니다. (GCM은 매번 다른 IV를 사용하기 때문에) 매번 암호화할 때마다 결과값이 달라져 DB 레벨에서 직접 비교가 불가능하기 때문입니다.  

이를 해결하기 위해 검색에 사용되는 컬럼은 **별도의 검색용 해시 컬럼을 만들고 인덱싱**하는 방식을 사용했습니다.  

이때 단순히 `SHA-256(전화번호)` 값을 저장하면 안 됩니다. 해시는 단방향이지만, 입력값의 경우의 수가 적으면 가능한 후보를 전부 대입하여 원본을 알아낼 수 있기 때문입니다.   

그래서 검색용 해시는 애플리케이션만 알고 있는 **비밀키를 섞은 HMAC-SHA256**으로 생성했습니다. 키가 DB 바깥에 있기 때문에, DB 유출만으로는 후보값의 해시를 계산하는 것이 불가능합니다.   

```
users 테이블
├── email         VARCHAR(500)  -- AES-256으로 암호화
├── name          VARCHAR(500)  -- AES-256으로 암호화
├── name_hash     VARCHAR(64)   -- HMAC-SHA256
├── phone_number  VARCHAR(500)  -- AES-256으로 암호화
├── phone_hash    VARCHAR(64)   -- HMAC-SHA256
```

이름으로 사용자를 검색해야 할 경우, 입력값을 저장할 때와 동일한 키, 동일한 정규화 규칙으로 해시한 뒤 `name_hash` 컬럼과 비교(`=`)하면 됩니다.

다만 이 방식은 **검색이 필요한 컬럼마다 해시 컬럼이 하나씩 추가되어 테이블의 컬럼 수가 늘어난다는 단점**이 있습니다. 또한 해시 기반 검색이기 때문에 **완전 일치 검색만 가능**하며, `LIKE` 검색 같은 부분 검색은 불가능합니다.  
...
기본 주소만으로는 상세 주소(동/호수)를 알 수 없어 개인을 특정하기 어렵습니다.  
...
> 다만 기본 주소에는 도로명과 건물번호까지 포함되기 때문에, 통계 목적만 놓고 보면 필요 이상의 정보를 평문으로 두는 셈입니다.   
> 시/도, 시/군/구만 별도 컬럼으로 추출해 통계에 사용하고 기본 주소는 암호화하는 방식도 고려하고 있습니다.
...
OAuth 정보를 저장하는 `SocialInfo` 클래스에는 `socialId` 필드를 아예 없애고, HMAC-SHA256을 거친 해시값만 저장하도록 socialIdHash 필드를 구성했습니다.
...
평문 `socialId`를 저장하지 않기 때문에, DB가 유출되더라도 소셜 계정 식별자가 그대로 노출되지는 않습니다. `socialId` 역시 경우의 수가 제한적일 수 있으므로, 단순 SHA-256이 아닌 HMAC을 사용했습니다.

> **핵심: 원본을 꺼내 써야 하면 양방향 암호화, 식별/검색만 하면 단방향 해시**  
> **단, 입력값의 경우의 수가 적다면 해시에 반드시 비밀키도 섞어야 함**
...
이를 통해 IP별, 엔드포인트별로 버킷을 독립적으로 관리할 수 있습니다. 결과적으로 특정 IP가 제한 기준을 초과해 차단되더라도, 해당 IP의 다른 API 호출이나 다른 IP의 요청에는 영향을 주지 않습니다.   
...
| 등급 | 대상 | 제한 | 이유 |
|------|------|------|------|
| CRITICAL | 로그인, SMS 인증, 결제 | 1분에 3~5회(SMS 인증은 5분에 3회) | 공격 차단 및 외부 비용 발생 |
| HIGH | 관리자 API, 주문 | 1분에 10~30회 | 대량 조회 차단 및 DB 부하 방지 |
| NORMAL | 상품 검색, 갤러리 | 1분에 40~60회 | 단일 IP의 과도한 요청 차단 |
...
| 항목 | 핵심 원칙 |
|------|----------|
| ID 설계 | 접근 권한 검증 / 외부 노출 ID는 예측이 어려운 식별자로 |
