---
title: "[Java] HashMap에서 키 타입에 따라 메모리 사용량이 달라지는 이유"
...
온라인 저지의 측정값에는 여러 요인이 영향을 줄 수 있어 모든 메모리 차이가 키 타입에서 발생했다고 단정할 수는 없지만, 키를 표현하는 방식이 메모리 사용량에 큰 영향을 줄 수 있음을 확인했습니다.   
...
> Java 9부터 `String`은 기본적으로 **Compact Strings**를 사용합니다.   
> 기존에는 문자열을 문자당 2바이트를 사용하는 **char[]**에 저장했지만, Java 9부터는 문자열의 문자 구성에 따라 **byte[]**에 저장하는 방식으로 변경해 메모리 사용량을 줄입니다.
> - 문자열이 LATIN1으로 표현 가능한 문자(영어, 숫자 등)만 포함하면 `coder`에 `LATIN1`을 기록하고, 각 문자를 `byte[]`에 1바이트씩 저장
> - LATIN1으로 표현할 수 없는 문자가 포함되면 `coder`에 `UTF16`을 기록하고, UTF-16 코드 단위당 2바이트씩 저장   
> 
> 참고로 **[디지털 로직 패턴 검사](https://www.codetree.ai/ko/frequent-problems/hsat/problems/check-digital-logic-pattern){:target='_blank'}**의 문자열은 `0`과 `1`로만 구성되어 있으므로 `LATIN1`으로 저장됩니다.   
> 따라서 길이 60의 문자열 데이터는 길이 60의 `byte[]`에 저장됩니다.
...
두 방식 모두 `HashMap`, `table`, `Node`라는 기본 구조를 공통으로 사용하며, 메모리 차이의 주된 원인은 `Node.key`가 참조하는 객체 구조입니다.   
...
앞서 언급한 온라인 저지의 메모리 사용량(`176MB → 16MB`)과 JOL의 측정값은 **측정 대상과 방식이 다르므로 서로 다른 수치로 봐야 합니다.**   
여기서는 동일한 조건에서 두 `HashMap`의 객체 그래프를 측정하여, 키를 표현하는 방식에 따라 메모리 사용량이 어떻게 달라지는지 비교해 보겠습니다.   

JOL의 `GraphLayout`을 사용하면 지정한 객체에서 참조를 따라 도달할 수 있는 모든 객체의 개수와 전체 메모리 크기를 측정할 수 있습니다.   
...
> ##### **`TreeNode`가 발생한 이유**   
> `String` 키를 사용한 맵에서는 **이번 실험에서 생성한 60자리 이진 문자열 키의 해시값이 일부 버킷에 집중되면서 해당 버킷이 트리 구조로 변환되었고,** 이에 따라 일부 엔트리가 `TreeNode`로 저장되었습니다.   
> 그 결과 `Node` 계열의 객체 그래프 크기에 56,400바이트의 차이가 발생했습니다.   
